...
with open('output/output_final_fix.json', 'r') as f:
    output = json.load(f)
...
print("="*80)
print("OFF_DAYS VERIFICATION - FINAL FIX")
print("="*80)
print(f"assignments OFF_DAYs: {len(assignment_off_days)}")
print(f"employeeRoster OFF_DAYs: {len(roster_off_days)}")
print(f"rosterSummary OFF_DAYs: {output['rosterSummary']['byStatus']['OFF_DAY']}")
...
if missing:
    ...
    print("\n⚠️  Missing OFF_DAYs by employee:")
    for emp_id, dates in sorted(missing_by_emp.items()):
        print(f"  {emp_id}: {len(dates)} missing")
else:
    print("\n✅ ALL OFF_DAYs present in both assignments and employeeRoster!")
